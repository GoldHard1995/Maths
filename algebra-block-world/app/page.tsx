...
import { ArrowLeft, ArrowRight, BookType, Brackets, Check, Clock3, Combine, House, ListOrdered, MoveHorizontal, Parentheses, Replace, RotateCcw, Sigma, Trophy } from 'lucide-react';
...
export default function Home(){
 ...
 return <main className={`world ${session||showLeaderboard?'playing':''}`}>
  <header className="topbar"><a className="hub-home" href={hubUrl}><House/>返回主畫面</a><button className="brand" disabled={Boolean(session?.finished&&connected&&!uploaded)} onClick={goHome}><span className="brand-icon"><MoveHorizontal/></span>代數方塊世界</button><span className="top-note">中一數學 · 代數</span></header>
