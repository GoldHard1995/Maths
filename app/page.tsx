...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    try {void Promise.resolve(context.registerTool({
      ...
      execute(input:unknown){
        ...
        const game=input.game as GameId;flushSync(()=>start(game));return {game,question:1,totalQuestions:16};
      },
  ...
  return <main className={`world ${session?'playing':''}`}>
    ...
    <section className={`play-area ${active?.color}`}><div className="play-nav"><Button variant="secondary" className="block-btn secondary" onClick={()=>dispatch({type:'home'})}><ArrowLeft/>返回選關</Button><div className="play-name">{active&&<active.icon/>}<h1>{active?.title}</h1></div><span className="elapsed"><Clock3/>已練習 {time}</span></div>
      {session.finished?<div className="game-panel finish"><div className="finish-check"><Check/></div><span className="task-label">這次探索完成了</span><h2>每一步，都更有把握。</h2><p>你完成了「{active?.title}」的全部 {session.questions.length} 個任務。</p><div className="result-grid"><div><strong>{session.firstTry}</strong><span>首次作答全對</span></div><div><strong>{session.questions.length-session.firstTry}</strong><span>練習後答對</span></div><div><strong>{time}</strong><span>本次練習時間</span></div></div><p className="result-note">需要提示也沒關係，理解比速度更重要。結果只在本次畫面顯示。</p><div className="answer-options"><Button className="block-btn" onClick={()=>start(session.game)}><RotateCcw/>再玩一次</Button><Button className="block-btn secondary" variant="secondary" onClick={()=>dispatch({type:'home'})}>選其他遊戲<ArrowRight/></Button></div></div>:
      <div className="game-panel"><div className="round-top"><span>任務 <b>{String(session.index+1).padStart(2,'0')}</b> / {session.questions.length}</span><span>{session.game==='brackets'?'−20 至 +20':'−10 至 +10'}</span></div><Progress aria-label="本局已完成任務" value={(session.index+(session.solved?1:0))/session.questions.length*100}/><GameBoard key={`${session.game}-${session.startedAt}-${session.index}`} session={session} answer={value=>dispatch({type:'answer',value})}/><div className="round-bottom"><span>{elapsed>=600?'已練習 10 分鐘，你可以繼續或返回選關。':'不用趕時間，想清楚再前進。'}</span>{session.solved&&<Button className="block-btn" onClick={()=>dispatch({type:'next'})}>{session.index===session.questions.length-1?'查看本次結果':'下一個任務'}<ArrowRight/></Button>}</div></div>}
    </section>}
  </main>;
}
