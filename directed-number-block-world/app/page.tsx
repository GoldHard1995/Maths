...
import { fetchPersonalBests, formatDuration, validStudent, type ClassName, type PersonalBest } from '@/lib/leaderboard';
const games = [
...
export default function Home() {
  ...
  return <main className={`world ${session||showLeaderboard?'playing':''}`}>
    ...
    <section className={`play-area ${active?.color}`}><div className="play-nav"><Button variant="secondary" className="block-btn secondary" disabled={session.finished&&!uploaded} onClick={goHome}><ArrowLeft/>返回選關</Button><div className="play-name">{active&&<active.icon/>}<h1>{active?.title}</h1></div><span className="elapsed"><Clock3/>已練習 {time}</span></div>
      {session.finished?<div className="game-panel finish"><div className="finish-check"><Check/></div><span className="task-label">這次探索完成了</span><h2>每一步，都更有把握。</h2><p>你完成了「{active?.title}」的全部 {session.questions.length} 個任務。</p><div className="final-score"><span>本次得分</span><strong>{score(session)}<small> / {session.questions.length*10} 分</small></strong></div><div className="result-grid"><div><strong>{session.firstTry}</strong><span>首次作答全對</span></div><div><strong>{session.questions.length-session.firstTry}</strong><span>練習後答對</span></div><div><strong>{time}</strong><span>遊戲時間</span></div></div><p className="result-note">每題首次全對 10 分，重試後完成 5 分。</p><ScoreUpload gameId={session.game} score={score(session)} maxScore={session.questions.length*10} elapsedSeconds={elapsed} firstTryCorrect={session.firstTry} wrongAttempts={session.errors} onUploaded={(identity,best)=>{setUploaded(true);setLastStudent(identity);if(best){setPersonalBests(current=>({...current,[session.game]:best}));setPersonalBestStatus('ready')}}}/><div className="answer-options"><Button className="block-btn" disabled={!uploaded} onClick={()=>start(session.game)}><RotateCcw/>再玩一次</Button><Button className="block-btn secondary" variant="secondary" disabled={!uploaded} onClick={goHome}>選其他遊戲<ArrowRight/></Button></div></div>:
