...
function Compare({s,answer}:{s:Session;answer:(a:string)=>void}) {
  const q=s.questions[s.index];return <><span className="task-label">選擇正確的大小符號</span><h2 className="compare-equation"><span>{signed(q.a)}</span><span className="missing">{s.solved?relation(q):'?'}</span><span>{signed(q.b)}</span></h2><div className="answer-options">{['>','<','='].map(a=><Button className="block-btn choice symbol" key={a} disabled={s.solved} onClick={()=>answer(a)} aria-label={a==='>'?'大於':a==='<'?'小於':'等於'}>{a}</Button>)}</div>{s.stageErrors>0&&<div className="support-line"><p>越右的數越大；同一位置的數相等。</p><NumberLine marks={[q.a,q.b]}/></div>}</>;
}
