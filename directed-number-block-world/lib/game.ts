...
export type Session={game:GameId;questions:Question[];index:number;stage:number;errors:number;stageErrors:number;firstTry:number;questionErrors:number;solved:boolean;finished:boolean;feedback:string;startedAt:number;completedAt:number|null};
...
export function startSession(game:GameId,questions=makeQuestions(game),now=Date.now()):Session{return {game,questions,index:0,stage:0,errors:0,stageErrors:0,firstTry:0,questionErrors:0,solved:false,finished:false,feedback:'',startedAt:now,completedAt:null}}
...
export function submit(s:Session,answer:string,now=Date.now()):Session{if(s.finished||s.solved)return s;const q=s.questions[s.index];if(answer!==expected(s)){let hint='';if(s.game==='locate')hint=legacy(q).a===0?'零是正負數的分界，找找數線中央。':`從零開始，向${legacy(q).a<0?'左':'右'}找 ${Math.abs(legacy(q).a)} 格。`;else if(s.game==='compare')hint='看一看數線：越右的數越大。';else if(s.game==='move')hint='留意加減號和括號內數字的正負號。';else if(s.game==='brackets')hint=s.stage===0?'括號前是減號時，括號內的數要取相反數。':'由左至右逐步計算。';else if(s.game==='multiply'||s.game==='divide')hint=ruleHint(q as SignedQuestion)+((q as SignedQuestion).values.length===3&&s.game==='divide'?' 連除要由左至右計算。':'');else {const m=q as MixedQuestion;hint=m.level===0?'先乘除，後加減。':s.stage<2?'先計算最內層括號。':'同級運算要由左至右計算。'}return {...s,errors:s.errors+1,questionErrors:s.questionErrors+1,stageErrors:s.stageErrors+1,feedback:`再試一次。${hint}`}}if(s.stage<lastStage(s))return {...s,stage:s.stage+1,stageErrors:0,feedback:s.game==='multiply'||s.game==='divide'?'符號正確！接着輸入完整答案。':s.game==='mixed'?(s.stage%2===0?'選擇正確！輸入這部分的結果。':'計算正確！繼續選擇下一步。'):s.game==='move'?'這一步正確，繼續下一步。':'拆括號正確！接着計算答案。'};const finished=s.index===s.questions.length-1;return {...s,solved:true,finished,completedAt:finished?now:null,firstTry:s.firstTry+(s.questionErrors===0?1:0),feedback:`答對了！這題獲得 ${s.questionErrors===0?10:5} 分。`}}
...
export function reducer(s:Session|null,a:Action):Session|null{if(a.type==='home')return null;if(a.type==='start')return a.session;if(!s)return s;return a.type==='answer'?submit(s,a.value):nextQuestion(s)}
