...
const boards: { id: BoardId; label: string }[] = [
  { id: 'simple', label: '簡易方程' },
  { id: 'like-terms', label: '含同類項' },
  { id: 'brackets', label: '含括號' },
  { id: 'fractions', label: '含分數' },
  { id: 'form-equation', label: '建立方程' },
  { id: 'applications', label: '方程應用' },
...
export default function LeaderboardPanel({
  ...
  const [board, setBoard] = useState<BoardId>('simple');
    ...
    fetchLeaderboard(board, classFilter, student)
      ...
      .catch(
        (reason) =>
          active &&
          setError(
            reason instanceof Error ? reason.message : '未能讀取排行榜。',
          ),
      );
    return () => {
      active = false;
  ...
  return (
    <section className="leaderboard-page">
      ...
      <div className="game-panel leaderboard-panel">
        <div className="board-tabs" role="tablist" aria-label="排行榜類別">
          {boards.map((item) => (
            <button
              key={item.id}
              role="tab"
        ...
        <div className="class-tabs" aria-label="班別篩選">
          {(['ALL', ...leaderboardClasses] as const).map((item) => (
            <button
              key={item}
        ...
        {!leaderboardUrl && (
          <div className="board-message">
            排行榜尚未連接 Google Sheets。完成 Apps Script 部署後即可使用。
          </div>
        )}
        {leaderboardUrl && !data && !error && (
          <div className="board-message">正在讀取排行榜……</div>
        )}
        ...
        {data && (
          <>
            ...
            <div
              ...
              role="table"
              aria-label={`${boards.find((item) => item.id === board)?.label}排行榜`}
            >
              ...
              {data.rankings.length ? (
                data.rankings.map((entry) => (
                  <div
                    ...
                    role="row"
                    key={`${entry.className}-${entry.studentNo}`}
                  >
                ...
                <div className="board-message">這個排行榜暫時未有成績。</div>
              )}
            </div>
            {data.self &&
              !data.rankings.some(
                (entry) =>
                  entry.className === data.self?.className &&
                  entry.studentNo === data.self?.studentNo,
              ) && (
                <div className="self-rank">
                  <span>你的名次</span>
                  <strong>第 {data.self.rank} 名</strong>
                  <span>
                    {data.self.className}　{data.self.studentNo} 號
                  </span>
                  <span>
                    {data.self.score} 分　
                    {formatDuration(data.self.elapsedSeconds)}
                  </span>
                </div>
              )}
          </>
