...
export type LinearValue = {
  constant: Fraction;
  coefficients: Record<Variable, Fraction>;
};
...
const multiplyFraction = (a: Fraction, b: Fraction) =>
  fraction(a.n * b.n, a.d * b.d);
const divideFraction = (a: Fraction, b: Fraction) =>
  fraction(a.n * b.d, a.d * b.n);
const zero = () => fraction(0);
const constant = (value: Fraction): LinearValue => ({
  constant: value,
  coefficients: { x: zero(), y: zero(), a: zero(), b: zero() },
});
const letter = (name: Variable): LinearValue => ({
  ...constant(zero()),
  coefficients: { ...constant(zero()).coefficients, [name]: fraction(1) },
});
const add = (left: LinearValue, right: LinearValue): LinearValue => ({
  ...
  coefficients: Object.fromEntries(
    variables.map((name) => [
      name,
      addFraction(left.coefficients[name], right.coefficients[name]),
    ]),
  ) as Record<Variable, Fraction>,
});
const negate = (value: LinearValue): LinearValue => ({
  ...
  coefficients: Object.fromEntries(
    variables.map((name) => [
      name,
      fraction(-value.coefficients[name].n, value.coefficients[name].d),
    ]),
  ) as Record<Variable, Fraction>,
});
const subtract = (left: LinearValue, right: LinearValue) =>
  add(left, negate(right));
const isConstant = (value: LinearValue) =>
  variables.every((name) => value.coefficients[name].n === 0);
const scale = (value: LinearValue, factor: Fraction): LinearValue => ({
  constant: multiplyFraction(value.constant, factor),
  coefficients: Object.fromEntries(
    variables.map((name) => [
      name,
      multiplyFraction(value.coefficients[name], factor),
    ]),
  ) as Record<Variable, Fraction>,
});
...
const divide = (left: LinearValue, right: LinearValue) => {
  if (!isConstant(right))
    throw new ParseFailure('unsupported', '除數不可含有字母。');
  return scale(left, divideFraction(fraction(1), right.constant));
};

class ParseFailure extends Error {
  kind: 'incomplete' | 'unsupported';
  constructor(kind: 'incomplete' | 'unsupported', message: string) {
    super(message);
    this.kind = kind;
  }
}
...
type Token = {
  kind: 'number' | 'variable' | 'operator' | 'left' | 'right';
  value: string;
};
...
class Parser {
  ...
  parse() {
    ...
    if (this.index !== this.tokens.length)
      throw new ParseFailure('incomplete', '請檢查算式格式。');
    return value;
  }
  ...
  private term(): LinearValue {
    ...
    while (true) {
      if (this.peek('operator', '*') || this.peek('operator', '/')) {
        ...
        value =
          operator === '*' ? multiply(value, right) : divide(value, right);
      } else if (
        this.peek('number') ||
        this.peek('variable') ||
        this.peek('left')
  ...
  private factor(): LinearValue {
    ...
    if (this.peek('number'))
      return constant(fraction(Number(this.take().value)));
    if (this.peek('variable')) return letter(this.take().value as Variable);
    if (this.peek('left')) {
      this.take();
      const value = this.expression();
      if (!this.peek('right'))
        throw new ParseFailure('incomplete', '括號尚未完成。');
      this.take();
      return value;
    }
...
export type ParseResult =
  | { ok: true; value: LinearValue }
  | { ok: false; kind: 'incomplete' | 'unsupported'; message: string };
...
export function equivalent(left: string, right: string) {
  ...
  return (
    same(a.value.constant, b.value.constant) &&
    variables.every((name) =>
      same(a.value.coefficients[name], b.value.coefficients[name]),
    )
  );
}

export function equationEquivalent(left: string, right: string) {
  const parseEquation = (raw: string) => {
    const parts = raw.replaceAll('＝', '=').split('=');
    if (parts.length !== 2) return null;
    const lhs = parseExpression(parts[0]),
      rhs = parseExpression(parts[1]);
    if (!lhs.ok || !rhs.ok) return null;
    const subtractFraction = (a: Fraction, b: Fraction) =>
      fraction(a.n * b.d - b.n * a.d, a.d * b.d);
    return [
      subtractFraction(lhs.value.constant, rhs.value.constant),
      ...variables.map((name) =>
        subtractFraction(
          lhs.value.coefficients[name],
          rhs.value.coefficients[name],
        ),
      ),
    ];
  };
  const a = parseEquation(left),
    b = parseEquation(right);
  if (!a || !b) return false;
  const nonZero = a.findIndex((value) => value.n !== 0);
  if (nonZero < 0) return b.every((value) => value.n === 0);
  const ratio = divideFraction(b[nonZero], a[nonZero]);
  return a.every((value, index) => {
    const scaled = multiplyFraction(value, ratio),
      target = b[index];
    return scaled.n === target.n && scaled.d === target.d;
  });
}

export function parseFraction(raw: string): Fraction | null {
  const source = raw.replaceAll('−', '-').trim();
  if (/^-?\d+$/.test(source)) return fraction(Number(source));
...
const compact = (raw: string) =>
  raw
    ...
    .replaceAll('÷', '/')
    .replace(/\s+/g, '');
const count = (raw: string, pattern: RegExp) =>
  compact(raw).match(pattern)?.length ?? 0;
...
function collected(raw: string) {
  ...
  const groups = terms.map(
    (term) => variables.find((name) => term.includes(name)) ?? 'constant',
  );
...
export function followsForm(raw: string, rule?: FormRule) {
  ...
  const source = compact(raw);
  if (
    rule.parentheses !== undefined &&
    count(source, /\(/g) !== rule.parentheses
  )
    return false;
  if (
    rule.multiplyDivide !== undefined &&
    count(source, /[*/]/g) !== rule.multiplyDivide
  )
    return false;
  if (rule.collected !== undefined && collected(source) !== rule.collected)
    return false;
  return (rule.requiredFragments ?? []).every((fragment) =>
